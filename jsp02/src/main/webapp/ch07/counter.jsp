...
<body>
	...
	if (count == null || count.equals("")) { //처음 방문하면 쿠키 신규생성
		...
		if (period > 3 * 1000) { //카운트 증가
			intCount++;
			response.addCookie(new Cookie("count", Integer.toString(intCount))); //방문시간 업데이트
			response.addCookie(new Cookie("visit_time", Long.toString(now_time)));
		}
